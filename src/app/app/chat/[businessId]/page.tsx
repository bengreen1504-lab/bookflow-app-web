import { notFound } from "next/navigation";
import { requireUser } from "@/lib/data/session";
import { sendMessageAction } from "@/lib/actions/chat";

export default async function ChatPage({
  params,
}: {
  params: Promise<{ businessId: string }>;
}) {
  const { businessId } = await params;
  const { supabase, user } = await requireUser();

  const { data: business } = await supabase
    .from("businesses")
    .select("name")
    .eq("id", businessId)
    .single();
  if (!business) notFound();

  const { data: messages } = await supabase
    .from("messages")
    .select("sender, body, created_at")
    .eq("business_id", businessId)
    .eq("customer_id", user.id)
    .order("created_at");

  return (
    <div className="max-w-[560px] mx-auto flex flex-col h-[calc(100vh-64px-64px)]">
      <h1 className="text-lg font-extrabold text-ink mb-4">{business.name}</h1>

      <div className="flex-1 overflow-y-auto flex flex-col gap-2.5 mb-4">
        {(messages ?? []).map((m, i) => (
          <div
            key={i}
            className={`max-w-[75%] px-3.5 py-2.5 rounded-2xl text-[13.5px] ${
              m.sender === "customer"
                ? "self-end bg-teal text-white"
                : "self-start bg-black/[0.04] text-ink"
            }`}
          >
            {m.body}
          </div>
        ))}
        {(messages ?? []).length === 0 && (
          <p className="text-gray text-[13.5px] text-center mt-8">
            Say hello to {business.name}.
          </p>
        )}
      </div>

      <form action={sendMessageAction} className="flex gap-2">
        <input type="hidden" name="business_id" value={businessId} />
        <input
          name="body"
          placeholder="Message…"
          required
          className="flex-1 px-3.5 py-3 rounded-xl border border-border text-[14px] outline-none focus:border-teal"
        />
        <button className="w-11 h-11 rounded-full bg-teal text-white flex items-center justify-center shrink-0">
          <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
            <path d="M2 10h16M18 10l-7-7M18 10l-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>
      </form>
    </div>
  );
}
